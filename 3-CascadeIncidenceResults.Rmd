...
# incidenceDF <- TidyLongitudinal(incidenceMatrix[resampleIndices[1:10], ], 2004)
...
beta1PlotSE <- ParameterPlot("beta1", posteriorFrameStart,
  posteriorFrameEnd, stats = TRUE,
  distLabels =  c("2004 posterior (blue)", "2014 posterior (red)")) +
  coord_cartesian(xlim= c(0, 1000 * max(posteriorFrameEnd$beta1)))
...
# Organize the data
startValues <- posteriorFrameStart %>%
  mutate(sim = 1:nrow(posteriorFrameStart),
         year = 2004)

allValues <- posteriorFrameEnd %>%
  mutate(sim = 1:nrow(posteriorFrameEnd),
         year = 2014) %>%
