...
undiagFile <- file.path(dataFolder, "undiagnosed_estimates_gbm.csv")
...
if (plotUndiagnosed) {
  ...
  undiagProp <- tbl_df(as.data.frame(t(undiagnosedMatrix)))
  colnames(undiagProp) <- c("estimate", "lower", "upper")
  undiagProp$year <- propUndiag$year
  ...
  # Create plot
  undiagPlot <- ggplot(data = undiagProp, aes(x = year)) +
    ...
    scale_colour_manual(name = "", 
                        labels = c("ECDC estimates", "COUNT estimate"),
                        values = c(asrcols[2], "black"),
                        limits = c("ecdc", "count")) +
  ...
  if (savePlots) {
    ...
    # Do the plots
    ggsave(file.path(figFolder, paste0("undiagnosedPercent-",
...
if (plotCascade) {
  ...
  if (savePlots) {
      ...
      ggsave(file.path(figFolder, paste0("ECDC_new_infections-",
                                      toString(analysisYear), ".png")),
...
cascadeStagePlots$unsuppressed <- plotCurrent$diagnosed +
  ggtitle(yLabels["unsuppressed"])
cascadeStagePlots$suppressed <- plotCurrent$diagnosed +
...
if (savePlots) {
  ...
  ggsave(file.path(figFolder, paste0("Cascades-",
                                     toString(analysisYear), ".png")),
         plot = plhivCombined, width = 20, height = 10, 
         units = "cm")
  
  # Save first cascade figure
  ggsave(file.path(figFolder, paste0("Cascade_Combined-",
                                     toString(analysisYear), ".png")),
         plot = cascadeFig1, width = 20, height = 20, 
         units = "cm")
  
  # Save second cascade figure
  ggsave(file.path(figFolder, paste0("Cascade_Stages-",
                                     toString(analysisYear), ".png")),
