...
# estimate. 
undiagnosedOption <- "ecdc"; # "count" or "linear" or "ecdc"

if (undiagnosedOption == "linear") {
  startYearProp <- c(0.2,0.13, 0.30) # c(best, lower, upper)
  # c(0.2,0.13, 0.30), c(0.3,0.2, 0.4), c(0.1, 0.06, 0.14)
  # Bestfit from Bayes, Medianfit from Bayes
  fileTag <- "Middle" # Add to file when saving results 
} else if (undiagnosedOption == "ecdc"){
  # Use results from the ECDC HIV Modelling Tool
  ecdcEstimates <- c(13.69, 13.06, 12.78, 12.43, 12.07, 
                     11.69, 11.38, 11.09, 10.84, 10.66, 10.59) / 100
  
  ecdcEstimatesLower <- c(12.91, 12.28, 12.09, 11.79, 11.43, 10.99, 10.60,
                          10.32, 10.08, 9.66, 9.30) / 100
  ecdcEstimatesUpper <- c(14.87, 14.26, 13.81, 13.39, 12.91, 12.44, 12.29,
                          12.25, 12.25, 12.29, 12.94) / 100
...
if (undiagnosedOption == "count") {
  ...
  # Use ECDC estimate for proportion undiagnosed and uncertainty from 
  # count
