...
cascadeBest$diagnoses <- newInfects$diagnoses # diagnoses_exclude_os
cascadeBest$infections <- newInfects$infections

# cascadeBest$infections <- c(641.393064,
# 644.110739,
# 644.888006,
# 646.980217,
# 653.488845,
...
# Percentage range in new infections for weighting
incUpper <- (newInfects$upper - newInfects$infections) / 
  newInfects$infections
incLower <- (newInfects$infections - newInfects$lower) / 
  newInfects$infections

incError <- 100 * pmax(incUpper, incLower) # calculate error range
...
points(years, cascadeBest$infections, col = "blue")
lines(years, apply(incidenceMatrix[resampleIndices, ], 2, median), 
      col = "blue")
lines(years, newInfects$upper, col = "green")
lines(years, newInfects$lower, col = "green")
...
hist(beta2end[resampleIndices], breaks = 100) # posterior
