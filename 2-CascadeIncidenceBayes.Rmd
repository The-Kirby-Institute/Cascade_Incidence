...
if (useSaved) {
  simTime <- "2016-03-23(17-56)" # in %Y-%m-%d(%H-%M) format 
                                 # or some other name
...
if (useSaved) {
  ...
  numBayesSamples <- 50000 # number of prior samples
  numCascadeSamples <- 1   # number of cascade samples for each param set
  ...
  f1 <- runif(numBayesSamples, 0.9, 15)        # uniform 
  ...
  # Priors for our error estimates
  errf1 <- runif(numBayesSamples, 0.9, 1.1)
  errf2 <- runif(numBayesSamples, 0.9, 1.1)
  errf3 <- runif(numBayesSamples, 0.9, 1.1)
  errf4 <- runif(numBayesSamples, 0.9, 1.1)
  
  # Sampling and run simulations -----------------------------------------
  
  cascadeSample <- cascadeAverage
  ...
  infectionsData <- cascadeAverage$infections
  ...
  for (bayesSample in 1:numBayesSamples) {
    ...
    # resultsMatrix <- incFunc(cascadeAverage, 
    #     beta[bayesSample], f1[bayesSample], 
    #     f3[bayesSample], f4[bayesSample])
    ...
    incidenceSample <- incFunc(cascadeAverage, 
        beta[bayesSample], f1[bayesSample], f3[bayesSample], f4[bayesSample], 
        err = c(errf1[bayesSample], errf2[bayesSample], errf3[bayesSample],
        errf4[bayesSample]))
    incidenceMatrix[bayesSample, ] < - incidenceSample
...
errBest <- c(errf1[bestFit], errf2[bestFit], errf3[bestFit], errf4[bestFit])
...
```{r quickresults}
hist(f1[resampleIndices])

apply(cascadeAverage[, 2:5], 1, function(x) x * betaBest/sum(x * betaBest))

incFunc(cascadeAverage, betaBest[2], betaBestRel[1], 
        betaBestRel[3], betaBest[4], 
        err = errBest)

cascadeAverage$infections
