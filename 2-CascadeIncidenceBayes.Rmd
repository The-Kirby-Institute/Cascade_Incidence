...
and applies a Bayesian Melding approach. 
...
# Specify f4 prior -------------------------------------------------------
# Trying lots of alternatives
f4opt1 <- list(dist = "beta", params = c(0.85, 10.76))   # orig approach
f4opt2 <- list(dist = "beta", params = c(1, 18))         # new beta

f4opt3 <- list(dist = "triangle", params = c(0, 0.3, 0.05)) # triangle
f4opt4 <- list(dist = "triangle", params = c(0, 0.45, 0.15)) # triangle
f4opt5 <- list(dist = "triangle", params = c(0, 0.6, 0.3)) # triangle

# Use a fitted lognormal distribution for the relative effect
# Currently a bit of a hack
# f4 <- rlnorm(2 * numBayesSamples, log(0.04), 1.16)
# f4 <- f4[f4 <= 1] # remove values greater than 1
# f4 <- f4[1:numBayesSamples] # take correct number of samples
...
if (useSaved) {
  ...
  simTime <- "2016-04-12(20-43)" # in %Y-%m-%d(%H-%M) format 
                                   # or some other name
  folder <- paste("Cascade_Bayes_", simTime, sep = "")
  inputFolder <- file.path(resultsFolder, folder)
  savedFile <- paste(simTime, "_Cascade_Bayes_Sims.rda", sep = "")
}
...
if (useSaved) {
  ...
  if (priorApproach == "independent") {
    ...
    # Set up beta2 prior
    beta2 <- PriorSample(numBayesSamples, "unif", c(beta[1], beta[2])) * 
      runif(numBayesSamples, errN2[1], errN2[2])
  ...
  # Create f4 prior -- always realtive to beta2
  ...
  if (saveSims){
    ...
    save(numBayesSamples, priorApproach, f4Prior,
         ...
         f1, f3, f4, incidenceMatrix, weights,
         file = file.path(outputFolder, paste(currTime, 
          "_Cascade_Bayes_Sims.rda", sep = "")))
  }
}
...
if (useSaved) {
  ...
  folder <- paste("Cascade_Bayes_", simTime, sep = "")
  inputFolder <- file.path(resultsFolder, folder)
  savedFile <- paste(simTime, "_Cascade_Bayes_Results.rda", sep = "")
  ...
  if (saveOutputs){
    ...
    save(cascadeBestBayes, numBayesSamples, priorApproach, f4Prior,
         beta1, beta2, beta3, beta4, f1, f3, f4,
