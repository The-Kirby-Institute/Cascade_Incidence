...
if (useSaved) {
  ...
  f3 <- runif(numBayesSamples, 0.05, 5)      # uniform
  # f3 <- rep(1, numBayesSamples)
  f4 <- rbeta(numBayesSamples, 0.85, 10.76)   # beta fitted to data
  beta <- runif(numBayesSamples, 0.005, 0.06)   # uniform: need to 
                                              # recalculate
...
hist(f3[resampleIndices] * beta[resampleIndices], breaks = 1000)
hist(f3 * beta, breaks = 1000)
...
priorFrame <- data.frame(beta1 = f1 * beta,
                         beta2 = beta,
                         beta3 = f3 * beta,
                         beta4 = f4 * beta)

posteriorFrame <- data.frame(beta1 = f1[resampleIndices] * 
                               beta[resampleIndices],
                             beta2 = beta[resampleIndices],
                             beta3 = f3[resampleIndices] *
                               beta[resampleIndices],
                             beta4 = f4[resampleIndices] * 
                               beta[resampleIndices])

ggplot(data = posteriorFrame, aes(x = beta1)) +
  geom_histogram(aes(y = (..count..)/sum(..count..)),
                     data = priorFrame,
                 binwidth = diff(range(beta1))/100) +
  geom_histogram(aes(y = (..count..)/sum(..count..)),
                     binwidth = diff(range(beta1))/100, alpha = 0.2) +
  coord_cartesian(xlim = range(beta1)) +
