...
numBayesSamples <- 5e6 # number of prior samples
...
simsSeed <- NULL # 54321 #Set to an integer to reproduce results
if (!is.null(simsSeed)) {
  set.seed(simsSeed)
} else {
...
# Specify f4 prior 
f4Prior <- SuppressedOption("option9")
...
if (useSaved) {
  ...
  currTime <- "2016-08-23(18-16)" # in %Y-%m-%d(%H-%M) format 
                                   # or some other name
...
if (useSaved) {
  ...
  if (saveSims){
    # Save the output
    save(numBayesSamples, priorApproach, simsSeed, f4Prior,
...
resampleSeed <- NULL #2357 Set to an integer to reproduce results
...
if (useSaved) {
  ...
  if (saveOutputs){
    ...
    save(cascadeBestBayes, numBayesSamples, priorApproach, simsSeed, 
         resampleSeed, f4Prior,
         betastart, beta1start, beta2start, beta3start, beta4start, 
         betaend, beta1end, beta2end, beta3end, beta4end,
         f1start, f2start, f3start,  
         f1end, f2end, f3end, 
...
hist(beta3end[resampleIndices], breaks = 100) # posterior
