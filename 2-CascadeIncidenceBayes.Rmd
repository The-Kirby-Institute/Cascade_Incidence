...
# Proportion infections for each stage
...
propFrame <- data.frame(sample = NULL, 
                        year = NULL, 
                        undiagnosed = NULL,
                        diagnosed = NULL,
                        unsuppressed = NULL,
...
for (yearprop in years) {
  ...
  loopFrame <- as.data.frame(t(apply(posteriorFrame, 1, 
    function(x) x * yearCascade / sum(x * yearCascade))))
  ...
  propFrame <- bind_rows(propFrame, loopFrame)
}

# Gather propFrame into long format
# propFrame <- propFrame %>%
#   select(sample, year, everything()) %>%
#   gather("stage", "proportion", 3:6) 

# Plot change in proportion acquired by stage aver time
ggplot(data = propFrame, 
       aes(x = 100 * undiagnosed, group = year, colour = factor(year))) + 
  geom_density(size = 1.2) + 
  scale_colour_brewer(palette = "RdYlBu", name = "Year") +  
  ylab("Density") + xlab("Percentage acquired from undiagnosed") +
