...
This script is designed to be as self contained as possible. 
...
saveCascadeSample <- TRUE  # save sample from cascade
saveRegression <- FALSE     # save regression results
...
if (saveCascadeSample){
  ggsave(file.path(outputFolder, 
                   paste(currTime, "_CountPlot.png", sep = "")))
}
...
# Set up a distribution for the reduction multiplicative factor
cascadeAdjust$beta2 <- runif(numRows, 0.5,1)
...
if (saveRegression) {
  save(coefUnadjustedOverall, cascadeAdjust, undetectAdjustAnalysis,
       artAdjustAnalysis, sampleAnalysis,
    file = file.path(outputFolder, paste(currTime, 
